...
; RUN: opt < %s -passes=hwasan -hwasan-kernel=1 -hwasan-recover=1 -hwasan-inline-all-checks=0 -hwasan-mapping-offset=12345678 -hwasan-use-short-granules=0 -S | FileCheck %s  --check-prefixes=OUTLINE
...
; CHECK: %[[B:[^ ]*]] = lshr i64 %[[A]], 57
; CHECK: %[[PTRTAG:[^ ]*]] = trunc i64 %[[B]] to i8
; CHECK: %[[C:[^ ]*]] = shl i64 %[[A]], 7
; CHECK: %[[UNTAG:[^ ]*]] = ashr i64 %[[C]], 7
...
; MATCH-ALL: %[[G:[^ ]*]] = icmp ne i8 %[[PTRTAG]], 127
; MATCH-ALL: %[[H:[^ ]*]] = and i1 %[[F]], %[[G]]
; MATCH-ALL: br i1 %[[H]], label {{.*}}, label {{.*}}, !prof {{.*}}

; NO-MATCH-ALL: br i1 %[[F]], label {{.*}}, label {{.*}}, !prof {{.*}}

; CHECK: call void asm sideeffect "ebreak\0Aaddiw x0, x11, 96", "{x10}"(i64 %[[A]])
; CHECK: br label
...
; OUTLINE: call void @llvm.hwasan.check.memaccess(ptr %[[SHADOW]], ptr %a, i32 58654752)
entry:
